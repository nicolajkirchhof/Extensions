...
merge_dists = sum((bsxfun(@minus, outer_ring, point_center).^2), 1).*tan(phi);
is_all_merged = false;
%%
while ~is_all_merged 
    %%
    ...
    edge_length_forward = sum((outer_ring(:,id_start)-outer_ring(:, id_next)).^2, 1);
    edge_length_backward = sum((outer_ring(:,id_previous)-outer_ring(:, id_end)).^2, 1);
...
%     cla,    Environment.draw(environment, false);
    ...
    is_inside_foreward = binpolygon(outer_ring(:,id_next), poly_forward);
    mb.drawPoint(outer_ring(:,id_next), 'color', 'g');
    ...
    is_inside_backward = binpolygon(outer_ring(:,id_previous), poly_backward);
        mb.drawPoint(outer_ring(:,id_previous), 'color', 'r');
...
ids_outer_ring_sorted = sort(ids_outer_ring);
